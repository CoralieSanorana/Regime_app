<?php $pageTitle = 'Export PDF - Codes'; ?>
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  ...
  <div class="sheet">
    <div class="head">
      <div>
        ...
        <div style="color:#475569;margin-top:4px;">Liste complète des codes de recharge</div>
      </div>
      <div class="meta">
        <div><strong>Date :</strong> <?= date('d/m/Y H:i') ?></div>
        <div><strong>Total :</strong> <?= count($codes) ?> codes</div>
      </div>
    </div>

    <div class="toolbar">
      <div style="color:#475569;font-size:14px;">Export imprimable de la base des codes</div>
    ...
    <table>
      ...
      <tbody>
        <?php foreach ($codes as $code): ?>
          <tr>
            <td>#<?= esc($code['id']) ?></td>
            <td><strong><?= esc($code['code_secret'] ?? '') ?></strong></td>
            <td><?= esc($code['montant'] ?? 0) ?> Ar</td>
            <td>
              <?php if (!empty($code['est_valide'])): ?>
